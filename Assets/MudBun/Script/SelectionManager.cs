...
namespace MudBun
{
  ...
  public class SelectionManager
  {
    ...
    internal static void NotifyRendererDisabled(MudRendererBase renderer)
    {
      ...
      s_selectedObjectsOnMouseDown = s_selectedObjectsOnMouseDown.Where(x => (x as GameObject)?.GetComponent<MudRendererBase>() != renderer).ToList();
      s_lastSelectedObjects = s_lastSelectedObjects.Where(x => (x as GameObject)?.GetComponent<MudRendererBase>() != renderer).ToList();
    }
    ...
    internal static void NotifyBrushDisabled(MudBrushBase brush)
    {
      ...
      s_selectedObjectsOnMouseDown = s_selectedObjectsOnMouseDown.Where(x => (x as GameObject)?.GetComponent<MudBrushBase>() != brush).ToList();
      s_lastSelectedObjects = s_lastSelectedObjects.Where(x => (x as GameObject)?.GetComponent<MudBrushBase>() != brush).ToList();
    }
      ...
      switch (e.type)
      {
        ...
        */
          ...
          if (!selection.Contains(rendererGo) 
              && !selection.Any(x => (x as GameObject)?.GetComponent<MudBrushBase>()?.Renderer == s_lastMouseDownRenderer))
          {
