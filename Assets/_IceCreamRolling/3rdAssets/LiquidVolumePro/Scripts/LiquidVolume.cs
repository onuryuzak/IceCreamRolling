...
namespace LiquidVolumeFX {
    ...
    public partial class LiquidVolume : MonoBehaviour {
        ...
        void CheckMeshDisplacement() {
            ...
            Mesh currentMesh1 = meshFilter.sharedMesh;
            if (_autoCloseMesh && currentMesh1 == closedMesh) return;
            if (!_autoCloseMesh && currentMesh1 == fixedMesh) return;

            // Backup original mesh
            if (originalMesh == null || !(originalMesh.name.Equals(currentMesh1.name))) {
                originalMesh = meshFilter.sharedMesh;
            }

            if (currentMesh1 != originalMesh) {
                RestoreOriginalMesh();
            }
